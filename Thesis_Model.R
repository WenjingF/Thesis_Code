...
for (i in 1:n_iter) {
  ...
  # Save results
  results31_2$mean_agb[i] <- predict_mean
  results31_2$ci_lower[i] <- ci_lower
  results31_2$ci_upper[i] <- ci_upper
}
...
n_iter <- 100
n_samples <- 100
results90 <- data.frame(mean_agb = numeric(n_iter),
                        ci_lower = numeric(n_iter),
                        ci_upper = numeric(n_iter))
...
for (i in 1:n_iter) {
  ...
  # Save results
  results90$mean_agb[i] <- predict_mean
  results90$ci_lower[i] <- ci_lower
  results90$ci_upper[i] <- ci_upper
}
...
# Calculate average bias and coverage
bias_90 <- mean(abs(results_90$bias))
coverage_90 <- mean(results_90$coverage)
...
# Initialize the results dataframe
results99 <- data.frame(mean_agb = numeric(n_iter),
                        ci_lower = numeric(n_iter),
                        ci_upper = numeric(n_iter))
...
for (i in 1:n_iter) {
  ...
  # Save results
  results99$mean_agb[i] <- predict_mean
  results99$ci_lower[i] <- ci_lower
  results99$ci_upper[i] <- ci_upper
}
...
# Calculate average bias and coverage
bias_99 <- mean(abs(results_99$bias))
coverage_99 <- mean(results_99$coverage)
#plot(final_model,x=mysample$Elevation)
# Output results
bias_99
coverage_99
